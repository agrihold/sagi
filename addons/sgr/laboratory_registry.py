...
class laboratory_registry(osv.osv):
    ...
    _description = 'laboratory_registry'
    ...
    _track = {
        'state': {
            'sgr.laboratory_registry_draft': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'draft',
            'sgr.laboratory_registry_requested': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'requested',
            'sgr.laboratory_registry_approved': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'approved',
            'sgr.laboratory_registry_rejected': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'rejected',
            'sgr.laboratory_registry_depreciated': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'depreciated',
            'sgr.laboratory_registry_cancelled': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'cancelled',
        },
    }
    _columns = {
...
Si un registro es del tipo operation &quot;replacement&quot;, también se debe seleccionar un registro &quot;operate_id&quot; pero para validar el segundo registor el primero debe estar en estado expirado"""),
        ...
        'laboratory_id': fields.many2one('res.partner', string='Laboratory', help=u"""Filtrar por partners del tipo laboratorio""", readonly=True, required=True, states={'draft': [('readonly', False)]}, context={'default_is_laboratory':'True','default_is_company':'True'}, domain=[('is_laboratory','=',True)]),
        'state': fields.selection(_states_, "State"),
        'operated_by_ids': fields.one2many('sgr.laboratory_registry', 'operate_id', string='Modified By', readonly=True), 
        'operate_id': fields.many2one('sgr.laboratory_registry', string='Modify'), 
    }
    ...
    def action_wfk_set_draft(self, cr, uid, ids, *args):
        ...
        for obj_id in ids:
            wf_service.trg_delete(uid, 'sgr.laboratory_registry', obj_id, cr)
            wf_service.trg_create(uid, 'sgr.laboratory_registry', obj_id, cr)
        return True

    def unlink(self, cr, uid, ids, context=None):
        sgr_registry_obj = self.pool.get('sgr.registry')
        data = self.read(cr, uid, ids, [ 'sgr_registry_id', ])
        res = super(laboratory_registry, self).unlink(cr, uid, ids, context=context)
        for item in data:
